...
module.exports = function (docName, schema, methods) {
  ...
  var _pValidateUpdate = function (data) {
    return when.promise(function (resolve, reject) {
        ...
        return when.promise(function (resolve, reject) {
          ...
          if (prop === '$currentDate') {
            ...
            for (z in data) {
              ...
              if (Object.prototype.toString.call(data[z]) ===
                '[object Object]') {
                if (data[z].$type !== 'timestamp' && data[z].$type !== 'date') {
                  return reject('$currentDate object need to have a valid ' +
                    '$type');
                }
              } else if (data[z] !== true) {
          ...
          if (prop === '$addToSet') {
            ...
            for (z in data) {
              ...
              if (Object.prototype.toString.call(data[z]) ===
                '[object Object]') {
                if (!data[z].$each ||
                  Object.prototype.toString.call(data[z].$each) !==
                  '[object Array]') {
                  return reject('To use $addToSet ' + z +
                    ' property as an object you need to use $each property.');
                } else {
                  data[z] = data[z].$each;
                }
              } else {
          ...
          if (prop === '$push') {
            ...
            for (z in data) {
              ...
              if (Object.prototype.toString.call(data[z]) ===
                '[object Object]') {
                if (Object.prototype.toString.call(data[z].$each) !==
                  '[object Array]') {
                  return reject('To use $push ' + z +
                    ' property as an object you need to use $each property.');
                } else {
                  data[z] = data[z].$each;
                }
      ...
      var compressNested = function (obj, path) {
        var compressed = '';
        ...
        while (Object.prototype.toString.call(temp) ===
          '[object Object]' && (compressed.slice(0, -1) !== path)) {
        ...
        if (_structIgnoreOps.indexOf(x) !== -1) {
          ...
          for (var y in data[x]) {
            ...
            if (y.toString().indexOf('.') !== -1) {
              ...
              nestedProps[x].hasNested.push(y);
            }
          }
        ...
        descriptors.forEach(function (d) {
          ...
          if (d.state === 'rejected') {
            ...
            for (x in d.value) {
              if (nestedProps[x]) {
                for (y in d.value[x]) {
                  for (i = 0, n = nestedProps[x].hasNested.length; i < n; i++) {
                    props = nestedProps[x].hasNested[i].split('.');
                    if (y === props[0]) {
                      tocompress = {};
                      tocompress[y] = d.value[x][y];
                      delete d.value[x][y];
                      _.extend(d.value[x],
                        compressNested(tocompress,
                          nestedProps[x].hasNested[i]));
                    }
                  }
                }
              }
            ...
            .then(function (db) {
                ...
                if (typeof options.justOne !== 'undefined' &&
                  ...
                  options.justOne = options.justOne === 'true' ||
                    options.justOne === '1' ||
                    options.justOne === 1 ? true : true;
                  options.justOne = options.justOne === 'false' ||
                    options.justOne === '0' ||
                    options.justOne === 0 ? false : true;
                } else {
   ...
   * - options <Object> opciones extras de insercion
   * - structOptions <Object> opciones de jay-schema
   * returns: <Promise> //Resultados son un array
   */
  this.pCreate =
    function pCreate(data, options, structOptions) {
      var promise;
      var defaultOptions = {
        w: 1
      };
            ...
            .then(function (db) {
                ...
                if (Object.prototype.toString.call(data) === '[object Array]') {
                  for (var i = 0, n = data.length; i < n; i++) {
                    structPromises.push(_schema.pMakeStruct(data[i], structOptions|| {}));
                    sPL++;
                  }
                }
                if (sPL > 0) {
                  when.settle(structPromises)
                    .then(function (descriptors) {
                      ...
                      collection
                        .insert(structs, options, function (err, result) {
                          if (err) {
                            return reject(err);
                          } else {
                  ...
                  _schema.pMakeStruct(data, structOptions || {})
                    .then(function (struct) {
                      collection
                        .insert(struct, options, function (err, result) {
                          if (err) {
                            return reject(err);
                          } else {
                            return resolve(result.ops);
